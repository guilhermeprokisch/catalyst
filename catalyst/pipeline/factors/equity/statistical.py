...
from catalyst.errors import IncompatibleTerms
from catalyst.pipeline.factors import CustomFactor
from catalyst.pipeline.filters import SingleAsset
from catalyst.pipeline.mixins import SingleInputMixin
from catalyst.pipeline.sentinels import NotSpecified
from catalyst.pipeline.term import AssetExists
from catalyst.utils.input_validation import expect_bounded, expect_dtypes
...
class RollingPearson(_RollingCorrelation):
    ...
    Parameters
    ----------
    base_factor : catalyst.pipeline.factors.Factor
...
class RollingLinearRegression(CustomFactor, SingleInputMixin):
    ...
    Parameters
    ----------
    dependent : catalyst.pipeline.factors.Factor
        The factor whose columns are the predicted/dependent variable of each
        regression with `independent`.
    independent : catalyst.pipeline.slice.Slice or catalyst.pipeline.Factor
    ...
    independent : catalyst.pipeline.Term with a numeric dtype
...
class RollingLinearRegressionOfReturns(RollingLinearRegression):
    ...
    :class:`catalyst.pipeline.factors.CustomFactor`.
