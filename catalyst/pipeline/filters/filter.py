...
from catalyst.lib.rank import is_missing
...
from catalyst.utils.numpy_utils import bool_dtype, repeat_first_axis
...
class Filter(RestrictedDTypeMixin, ComputableTerm):
    ...
    requested computation. For example, :meth:`catalyst.pipeline.Factor.top`
    ...
    operators (``<``, ``<=``, ``!=``, ``eq``, ``>``, ``>=``) of
    :class:`~catalyst.pipeline.Factor`. For example, a natural way to construct
...
class PercentileFilter(SingleInputMixin, Filter):
    ...
    Parameters
    ----------
    factor : catalyst.pipeline.factor.Factor
...
class CustomFilter(PositiveWindowLengthMixin, CustomTermMixin, Filter):
    ...
    :class:`~catalyst.pipeline.factors.factor.CustomFactor` for more details on
